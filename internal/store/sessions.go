...
const sessionColumns = `id, title, os_user, machine, project_dir, git_branch, model, status, last_tool,
	...
	started_at, last_seen_at, ended_at, remote_control, last_report_at, api_error_status`
...
func scanSession(sc scanner) (Session, error) {
	...
	err := sc.Scan(
		...
		&s.StartedAt, &s.LastSeenAt, &s.EndedAt, &s.RemoteControl, &s.ReportedAt, &s.APIErrorStatus,
	)
	return s, err
}
...
INSERT INTO sessions (
	...
	input_tokens, output_tokens, cache_creation_tokens, cache_read_tokens,
	started_at, last_seen_at, ended_at, last_report_at, remote_control, api_error_status
) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
ON CONFLICT(id) DO UPDATE SET
	...
	ended_at = excluded.ended_at`,
		...
		sess.StartedAt, sess.LastSeenAt, sess.EndedAt, sess.ReportedAt, sess.RemoteControl, sess.APIErrorStatus,
	)
	if err != nil {
