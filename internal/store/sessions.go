...
const sessionColumns = `id, title, os_user, machine, project_dir, git_branch, model, effort, status, last_tool,
...
func scanSession(sc scanner) (Session, error) {
	var s Session
	err := sc.Scan(
		&s.ID, &s.Title, &s.User, &s.Machine, &s.ProjectDir, &s.GitBranch, &s.Model, &s.Effort, &s.Status, &s.LastTool,
...
INSERT INTO sessions (
	id, title, os_user, machine, project_dir, git_branch, model, effort, status, last_tool,
	input_tokens, output_tokens, cache_creation_tokens, cache_read_tokens,
...
ON CONFLICT(id) DO UPDATE SET
	...
	ended_at = excluded.ended_at`,
		sess.ID, sess.Title, sess.User, sess.Machine, sess.ProjectDir, sess.GitBranch, sess.Model, sess.Effort, sess.Status, sess.LastTool,
